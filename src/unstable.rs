...
impl UnstableFeatures {
    ...
    pub fn new(features: impl IntoIterator<Item = UnstableFeature>) -> Self {
        Self::from_sorted_dedup(features.into_iter().collect())
    }

    /// Build from an already-collected `Vec`, normalizing it in place.
    ///
    /// The sort + dedup canonicalize the representation so the derived `Eq` is
    /// order- and duplicate-independent; lookups in `is_enabled` are a linear
    /// scan and do not rely on the ordering.
    fn from_sorted_dedup(mut enabled_features: Vec<UnstableFeature>) -> Self {
    ...
    pub fn from_names(names: impl IntoIterator<Item = impl AsRef<str>>) -> Result<Self, String> {
        let features = names
            ...
            .filter_map(|n| {
                let s = n.as_ref().trim();
                (!s.is_empty()).then(|| s.parse::<UnstableFeature>())
            })
            .collect::<Result<Vec<_>, _>>()?;

        // Normalize like `new` so the sorted + deduped invariant holds regardless
        // of constructor (e.g. `-Z imports -Z imports` collapses to one entry),
        // reusing the `Vec` we already collected for the `?` short-circuit.
        Ok(Self::from_sorted_dedup(features))
    }
}
...
mod tests {
    ...
    fn test_from_names_dedups_like_new() {
        ...
        let name = feature.to_string();
        // A repeated flag (e.g. `-Z imports -Z imports`) collapses to one entry,
        // equal to the canonical `new` (both share the sorted + deduped form).
